...
class pcorte(Tk):
	def __init__(self,s,c,ap,qi,t,C,espessura,count,sens):
		...
		self.iconbitmap('@gerarplanocorte_icon.xbm')
		...
		self.ca = Canvas(self,height=(self.winfo_screenheight())*0.75,width=(self.winfo_screenwidth())*0.85,bg="white")
	...
	def desenho(self,x,r):
		...
		f = 'snow'
		for j in range(len(self.solucao[r])): # Faixas
			if 'h' in self.solucao[r][j]:
				a = x0
				for k in range(len(self.solucao[r][j])-1): # Peças
