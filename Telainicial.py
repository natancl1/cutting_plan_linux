...
class tela(Tk):
	...
	def inserirpecas(self):
		...
		x.iconbitmap('@cadastrarpecas_icon.xbm')
		x.title('Cadastrar Peças')
		x.eval('tk::PlaceWindow %s center' % x.winfo_pathname(x.winfo_id()))
		x.mainloop()

	def inserirchapas(self):
		x = Janelainserirchapa.ListboxExe()
		x.iconbitmap('@cadastrarchapas_icon.xbm')
	...
	def conf(self):
		...
		x.iconbitmap('@iconfiguracoes_icon.xbm')
