...
class ListboxExe(Tk):
    ...
    def salvar(self):
        cont=0
        self.pecas=[]
        if self.list1.size() != 0 and self.list2.size() != 0 and self.list3.size() != 0 and self.list4.size() != 0 and self.list5.size() != 0:
            for i in range(0,min(self.list1.size(),self.list2.size(),self.list3.size(),self.list4.size(),self.list5.size())):
                if self.pecas==[]:
                    self.pecas.append([self.list1.get(i),self.list2.get(i),self.list3.get(i),self.list4.get(i),self.list5.get(i)])
                else:
                    for c in self.pecas:
                        if (c[1] == self.list2.get(i) and c[2] == self.list3.get(i)) or (c[2] == self.list2.get(i) and c[1] == self.list3.get(i)):
                            c[3]+=self.list4.get(i)
                            cont+=1
                    if cont==0:
                        self.pecas.append([self.list1.get(i),self.list2.get(i),self.list3.get(i),self.list4.get(i),self.list5.get(i)])
                    cont=0
        self.cp.execute('DELETE FROM pecas')
        self.cpecas.commit()
        for i in self.pecas:
            self.cp.execute('INSERT INTO pecas (id,comprimento,largura,quantidade,area) VALUES (?,?,?,?,?)',(i[0],i[1],i[2],i[3],i[4]))
        self.cpecas.commit()
    ...
    def inserir(self):
        ...
        self.janelainserir.iconbitmap('@cadastrarpecas_icon.xbm')
        self.janelainserir.title('Inserir Peças')
        wdt = 590
        hgt = 70
        self.janelainserir.geometry('%dx%d+%d+%d' % (wdt,hgt,((self.janelainserir.winfo_screenwidth()/2) - (wdt/2)),((self.janelainserir.winfo_screenheight()/2) - (hgt/2))))
...
if __name__ == "__main__":
    root = ListboxExe()
    root.iconbitmap('@cadastrarpecas_icon.xbm')
    root.title('Cadastrar Peças')
    wdtp = 800
    hgtp = 300
